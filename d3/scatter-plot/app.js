...
function ready(movies) {
  ...
  // Scales.
  const xExtent = d3.extent(scatterData, d => d.budget).map((d,i) => i===0 ? d*0.95 : d*1.05);
  const xScale = d3.scaleLog()
    .domain(xExtent)
    .range([0,width]);
  
  const yExtent = d3.extent(scatterData, d => d.revenue).map((d,i) => i===0 ? d*0.1 : d*1.1);
  ...
  // Draw x axis.
  const xAxis = d3.axisBottom(xScale).ticks(5).tickFormat(formatTicks).tickSizeInner(-height).tickSizeOuter(0);
  const xAxisDraw = svg.append('g').attr('class', 'x axis')
    .attr('transform', `translate(0, ${height})`).call(xAxis).call(addLabel, 'Budget', 25);
  ...
  const yAxisDraw = svg.append('g')
    .attr('class', 'y axis').call(yAxis).call(addLabel, 'Revenue', 5);
